...
class Location:
    ...
    def __repr__(self):
        repre = f"{self.__class__.__name__}(id={self.ID},x={self.x},y={self.y},shape={self.shape},depth={self.depth},"
        repre += f"capacity={self.capacity},quarantined={self.quarantined},quarantined_time={self.quarantined_time},"
        if self.shape == Shape.CIRCLE.value:
            repre += f"radius={self.radius},"
        elif self.shape == Shape.POLYGON.value:
            repre += f"boundary={self.boundary},"
        repre += f"exit=({self.exit[0]}|{self.exit[1]}),"

        repre += f"infectious={self.infectious},"
        if self.parent_location is None:
            repre += f"parent_id=-1,"
        else:
            repre += f"parent_id={self.parent_location.ID},"

        if self.override_transport is None:
            repre += f"override_transport=-1,"
        else:
            repre += f"override_transport={self.override_transport.ID},"

        repre += f"name={self.name})"
