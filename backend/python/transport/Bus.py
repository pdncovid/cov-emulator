...
class Bus(Transport):
    ...
    def initialize_locations(self, location):
        self.route_label[Bus.get_route_name(location, location)] = 0
        locs = [str(location)] + [str(l) for l in location.locations]

        pairs = list(itertools.combinations(locs, 2))
        n = len(pairs)
        for idx, pair in enumerate(pairs):
            self.route_label[Bus.get_route_name(pair[0], pair[1])] = idx // 3
            self.route_label[Bus.get_route_name(pair[1], pair[0])] = idx // 3
        if Bus.DEBUG:
    ...
    def remove_point_from_transport(self, point):
        ...
        if t is not None:
            rlabel = self.route_label[Bus.get_route_name(f, t)]
    ...
    def insert_into_vehicle(self, point, fr, to, t):
        rlabel = self.route_label[Bus.get_route_name(fr, to)]
        ...
        if not success:
            ...
            self._current_label = key
        if Transport.DEBUG:
            print(f"""Added person {point} going in {Bus.get_route_name(fr,
    ...
    def transport_point(self, idx, destination_xy, t):
        rlabel = self.route_label[Bus.get_route_name(self.points_source[idx], self.points_destination[idx])]
        vlabel = self.points_label[idx]
        point = self.points[idx]
        if t > self.vehicles_in_route_leaving_t[rlabel][vlabel]:
